...
1 So first made an unsupervised anomaly detection model using an Autoencoder(line 22) which learns what normal network is and if there is high reconstruction error it flags it as an anomaly.

2 Then created supervised model (line 73) which which predicts zoom freeze_count and call_rating based on network's latency and packet_loss.

3 Now we can detect both network abnormal behaviour and how it impacts zoom quality.
